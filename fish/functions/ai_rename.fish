...
function ai_rename --description "Renames a file using AI based on its content"
    ...
    if test -z "$original_file"
        echo "Usage: ai_rename [-y|--yes] <filename>" >&2
        echo "Example: ai_rename 'My Document.pdf'" >&2
    ...
    if test $anthropic_status -ne 0
        echo "Error: anthropic-cli failed with status $anthropic_status." >&2
    ...
    if test -z "$suggested_name"
        echo "Error: AI did not return a suggested name." >&2
        return 1
    end
    ...
    # Example for more chars: set sanitized_name (command string replace -ra '[<>:"/\\|?*]' '_' -- "$base_name")
    ...
    if test -e "$final_name"
        echo "Error: A file or directory named '$final_name' already exists. Aborting rename." >&2
        return 1
        # Alternative: Could prompt user here if they want to overwrite or add suffix
    end
    ...
    if test "$confirm_char" = "y" -o "$confirm_char" = "Y"
        ...
        if mv -- "$original_file" "$final_name"
            ...
            echo "Error: Failed to rename '$original_file' to '$final_name'. Check permissions or other issues." >&2
            # Try to capture mv error? Difficult with 'if'. $status might be checked after.
            return 1 # Failure
        end
    else
        echo "Rename cancelled by user."
        return 0 # User cancelled, not an error state for the script itself
    end
end
